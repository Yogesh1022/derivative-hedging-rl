...
class HistoricalDataLoader:
    ...
    def get_random_episodes(
        self, n_episodes: int = 100, episode_length: int = 100, seed: Optional[int] = None
    ) -> List[pd.DataFrame]:
        """
        ...
        if len(df) < episode_length:
            raise ValueError(f"Not enough data: need {episode_length} rows, have {len(df)}")
    ...
    def prepare_for_environment(self, data: pd.DataFrame, normalize: bool = True) -> np.ndarray:
        """
        ...
        feature_cols = ["SPY_Close", "SPY_returns", "SPY_volatility_20", "VIX", "risk_free_rate"]
