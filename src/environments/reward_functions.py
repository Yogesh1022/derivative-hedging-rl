...
class AsymmetricReward(BaseReward):
    ...
    def __init__(
        self, loss_multiplier: float = 2.0, gain_multiplier: float = 1.0, pnl_scale: float = 10.0
    ):
...
class SharpeReward(BaseReward):
    ...
    def __init__(self, window: int = 50, target_sharpe: float = 1.5):
        self.window = window
...
class VariancePenalizedReward(BaseReward):
    ...
    def __init__(self, variance_penalty: float = 0.5, window: int = 20):
...
class CompositeReward(BaseReward):
    ...
    def __init__(self, reward_functions: List[tuple]):
        ...
        self.reward_functions = [(fn, weight / total_weight) for fn, weight in reward_functions]
