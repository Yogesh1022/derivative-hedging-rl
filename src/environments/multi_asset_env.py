...
class MultiAssetHedgingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.current_asset in self.historical_data and hasattr(self, "data_slice"):
            ...
            if self.current_step < len(self.data_slice) - 1:
                ...
                self.sigma = self.data_slice.iloc[self.current_step + 1].get(
                    "Volatility_20", self.sigma
                )
        else:
            # Use GBM simulation
            dt = 1 / 252
    ...
    def black_scholes_delta(self):
        ...
        d1 = (np.log(self.S / self.K) + (self.r + 0.5 * self.sigma**2) * T) / (
    ...
    def black_scholes_theta(self):
        ...
        theta = -(self.S * norm.pdf(d1) * self.sigma) / (2 * np.sqrt(T)) - self.r * self.K * np.exp(
            -self.r * T
        ) * norm.cdf(d2)
