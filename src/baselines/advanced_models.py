...
class SABRHedging:
    ...
    def __init__(self, alpha: float = 0.05, beta: float = 0.5, rho: float = -0.3, nu: float = 0.4):
        """
    ...
    def implied_volatility(self, F, K, T):
        ...
        denominator = FK_mid * (
            1 + ((1 - self.beta) ** 2 / 24) * log_FK**2 + ((1 - self.beta) ** 4 / 1920) * log_FK**4
        )
...
class LocalVolatilityHedging:
    ...
    def build_surface(self, market_data):
        ...
        logger.info(
            f"✅ Built local vol surface: {len(self.strikes)} strikes × {len(self.maturities)} maturities"
        )
