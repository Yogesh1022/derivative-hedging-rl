...
class AgentTrainer:
    ...
    def train_with_curriculum(
        ...
        max_n_steps = max(
            ENV_CONFIGS.get(stage["difficulty"], {}).get("n_steps", 100) for stage in stages
        )
        print(
            f"Using n_steps={max_n_steps} for all curriculum stages (ensures consistent observation space)"
        )
